...
class Snailmail::Transcription
  def self.wav_to_text(url)
    ...
    #
    # Here I stream the recording url through ffmpeg to get flacc, and
    # simultaneously upload to google for analysis, all while praying it
    # actually works =D
    #

    json_result = JSON.parse %x{curl #{url} 2> /dev/null | ffmpeg -i pipe:0 -vn -sn -acodec flac -f flac pipe:1 | curl -v -X POST -H 'Content-Type: audio/x-flac; rate=8000' -H 'User-Agent: Mozilla/5.0 (Macintosh; Intel Mac OS X 10_6_8) AppleWebKit/535.7 (KHTML, like Gecko) Chrome/16.0.912.77 Safari/535.7' --data-binary @- "https://www.google.com/speech-api/v1/recognize?xjerr=1&client=chromium&lang=en-US&results=10"}
